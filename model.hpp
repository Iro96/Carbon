...
struct Model {
    Embedding emb; std::vector<TransformerBlock> blocks; Linear lm_head;
    Model(int vocab,int dim,int hidden,int layers,int heads):emb(vocab,dim),lm_head(dim,vocab){
    ...
    void step(float lr){emb.step(lr); for(auto &b:blocks)b.step(lr); lm_head.step(lr);}
    void save(const std::string&path){
        std::ofstream f(path,std::ios::binary); emb.save(f); for(auto &b:blocks)b.save(f); lm_head.save(f); f.close();
    }
    void load(const std::string&path){
        std::ifstream f(path,std::ios::binary); emb.load(f); for(auto &b:blocks)b.load(f); lm_head.load(f); f.close();
    }
};
